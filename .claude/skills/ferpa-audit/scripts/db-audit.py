...
def run_db_checks(conn, ferpa: dict, repo_root: Path) -> tuple[list[dict], str]:
    findings: list[dict] = []
                ...
                findings.append(
                    {
                        ...
                        "regulation": "§99.35 — disclosure for research / statistical purposes",
...
def main() -> None:
    ...
    config_path = repo_root / "ferpa-config.yaml"
    ...
    if not args.skip_db and psycopg2:
        cfg = load_db_config(repo_root)
        try:
            conn = psycopg2.connect(**cfg)
            try:
                db_findings, appendix_db = run_db_checks(conn, ferpa, repo_root)
            ...
            all_findings.append(
                {
                    ...
                    "regulation": "§99.3 — education records",
    ...
    elif not args.skip_db and not psycopg2:
        all_findings.append(
            {
                "severity": "Note",
